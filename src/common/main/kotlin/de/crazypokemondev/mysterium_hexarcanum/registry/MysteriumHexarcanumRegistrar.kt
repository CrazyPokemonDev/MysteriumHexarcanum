...
typealias RegistrarEntry<T> = MysteriumHexarcanumRegistrar<T>.Entry<out T>

abstract class MysteriumHexarcanumRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(MysteriumHexarcanum.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is MysteriumHexarcanumRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
